#!/bin/bash
# estimate vg with gcta with ld scaled GRM
# estimate vg with gcta with wganc
...
#output files
grmdir=${klemdir}/output/grm/gcta/GRMld/${model}
HEdir=${klemdir}/output/${model}/gcta/HE_GRMld
...
module load R  # slurm needs this!
Rscript GRMld.R ${filename} ${plinkdir} ${grmdir}

# estimate vg with gcta without adjustment
# feed special GRM.GZ
./gcta64 --HEreg \
--grm ${grmdir}/${filename} \
