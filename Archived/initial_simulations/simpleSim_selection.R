...
# simple simulation to validate hypothesis to test is there bias in vgb estimation
...
fstc = opt$fstc # fst of causal variants
fstn = opt$fstn # fst of non-causal variants
vgt = opt$h2 #desired total genetic variance 
vgb = opt$fstc * vgt #desired genetic variance in between
nloci = opt$nloci #number of loci, causal + non-causal
pcausal = opt$pcausal # proportion of causal variants
ncausal = round(pcausal*nloci) # get number of causal variants
venv = 1 - opt$h2 # environmental variance, keep Vp=1
...
geno1 = replicate(ncausal, rbinom(1000, 2, f1c))
#simulate genotypes for pop2
geno2 = replicate(ncausal, rbinom(1000, 2, f2c))
...
vwithin = sum(2*(bg^2)*f1c*(1 - f1c) + 2*(bg^2)*f2c*(1 - f2c))/2
...
geno.admix = cbind(as.data.table(geno.admix_c), as.data.table(geno.admix_n))

# fill the rest effect size with 0
...
lanc <- cbind(as.data.table(lanc_c), as.data.table(lanc_n))
lanc.t<-t(lanc)
