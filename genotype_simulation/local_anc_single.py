...
parser.add_argument("--time","-t",dest="time",help="time of generation to check ancestry",type=int,default=20,nargs="?")
...
#parser.add_argument("--start","-s",dest="start",help="start number of sample - must be a positive interger number (e.g. 50 or 500)",type=int,default=0,nargs="?")
#parser.add_argument("--end","-e",dest="end",help="end number of sample - must be a positive interger number (e.g. 50 or 500)",type=int,default=600,nargs="?")
parser.add_argument("--sample","-sa",dest="sample",help="number of sample - must be a positive interger number (e.g. 50 or 500)",type=int,default=0,nargs="?")
...
# output the dictionary for local ancestry
#for sample in range(start,end):
la=find_local_ancestry(sample = sample, time = time, ts=ts)
f = open(args.outpre+"_"+str(sample)+".txt","a")
for i in list(la.items()):  
    f.write(str(sample))
    f.write('\t')
    f.write(str(i))
    f.write('\n')
