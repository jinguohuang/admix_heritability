...
filename=$1
#name=$(echo "$filename" | cut -f 1,2 -d .) # for 10deme
name=$(echo "$filename" | cut -f 1 -d .) # for random
...
Rscript plot_admixture_dist_density_kstest.R ${name}_AT_maf0.05_pruned.2.Q ${name}_AT_maf0.05_pruned.fam ASW_ind_anc1.txt
