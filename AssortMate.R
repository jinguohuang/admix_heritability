...
set.seed(100) # keep this part same across P
...
geno1 <- matrix(rbinom(nloci*(n/2), 2, f1), byrow=T, nrow=(n/2), ncol=nloci)
geno2 <- matrix(rbinom(nloci*(n/2), 2, f2), byrow=T, nrow=(n/2), ncol=nloci)
...
ganc_meta[1,] = c(rep(1, (n/2)), rep(0, (n/2))) 
...
for (i in 1:t){ #generation of admixture
  ...
  ganc_meta[i+1,] = (ganc_meta[i,idxf] + ganc_meta[i,idxm])/2
}

# output the lanc of each round

for (i in 1:(t+1)){
  print(paste0("output lanc for generation ", i-1))
  ganc = ganc_meta[i,]
  ...
  filename <- paste0("admix", "_seed", seed, "_P", P, "_t", i-1) #for filename
  LAnc2Plink(lanc_adm, nloci, n, filename, FID="ADM", adm_prs)
  # write to file to record exp. and obs. value 
  sink(file=paste0("sim_",filename, "_exp_obs.txt"), type = "output")
  cat("vbetween.genic \t vbetween.obs \t P \t t \t seed \n")
  cat(vbetween.genic, "\t", vbetween.obs, "\t", P, "\t", i-1, "\t", seed, "\n")
  sink()  
}
